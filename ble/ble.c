...
typedef enum
{
	...
	DEVICE_IP_ADDRESS_HANDLE =
		ATT_CHARACTERISTIC_be3d7603_0ea0_4e96_82e0_89aa6a3dc19f_01_VALUE_HANDLE,
	SERVER_IP_ADDRESS_HANDLE =
		ATT_CHARACTERISTIC_be3d7604_0ea0_4e96_82e0_89aa6a3dc19f_01_VALUE_HANDLE,
	SERVER_PORT_HANDLE = ATT_CHARACTERISTIC_be3d7605_0ea0_4e96_82e0_89aa6a3dc19f_01_VALUE_HANDLE,
	//
	WIFI_SSID_USER_DESCRIPTION_HANDLE =
		ATT_CHARACTERISTIC_be3d7601_0ea0_4e96_82e0_89aa6a3dc19f_01_USER_DESCRIPTION_HANDLE,
	WIFI_PASSWORD_USER_DESCRIPTION_HANDLE =
		ATT_CHARACTERISTIC_be3d7602_0ea0_4e96_82e0_89aa6a3dc19f_01_USER_DESCRIPTION_HANDLE,
	DEVICE_IP_ADDRESS_USER_DESCRIPTION_HANDLE =
		ATT_CHARACTERISTIC_be3d7603_0ea0_4e96_82e0_89aa6a3dc19f_01_USER_DESCRIPTION_HANDLE,
	SERVER_IP_ADDRESS_USER_DESCRIPTION_HANDLE =
		ATT_CHARACTERISTIC_be3d7604_0ea0_4e96_82e0_89aa6a3dc19f_01_USER_DESCRIPTION_HANDLE,
	SERVER_PORT_USER_DESCRIPTION_HANDLE =
		ATT_CHARACTERISTIC_be3d7605_0ea0_4e96_82e0_89aa6a3dc19f_01_USER_DESCRIPTION_HANDLE,
} attribute_handle_t;

const int RAND_CHECK_NUM = 1639123567;
...
static void notify_ip_address_callback(void *context)
{
	...
	int err =
		att_server_notify(*notify->con_handle, DEVICE_IP_ADDRESS_HANDLE, notify->data, notify->len);
	if (err)
	{
...
static void state_entry_action(device_state_t state)
{
	...
	case DEVICE_WIFI_LINK_CONNECTED:
	{
		...
		if (con_handle != HCI_CON_HANDLE_INVALID)
		{
			...
			notify.data = wifi_setting.device_ip_address;
			notify.len = strlen(wifi_setting.device_ip_address);
	...
	case DEVICE_WIFI_LINK_DOWN:
	{
		...
		cyw43_cb_tcpip_set_link_down(&cyw43_state, 1);
		...
		wifi_setting.device_ip_address[0] = '\0';

		if (con_handle != HCI_CON_HANDLE_INVALID)
		{
			att_server_notify(con_handle, DEVICE_IP_ADDRESS_HANDLE,
							  (uint8_t *)&wifi_setting.device_ip_address,
							  strlen(wifi_setting.device_ip_address));
		}
		is_connected = 0;
		break;
	}
	case DEVICE_RUNNING:
		...
		is_connected = 1;
		LCD_print_next(" ", 0);
		LCD_print_next("Connecting to:", 0);
		strncpy(lcd_buff, wifi_setting.server_ip_address, strlen(wifi_setting.server_ip_address));
		lcd_buff[strlen(wifi_setting.server_ip_address)] = 0;
		LCD_print_next(lcd_buff, RED);
		LCD_print_next(" ", 0);
		LCD_print_next("wait...", GRAY);
		break;
	default:
		is_connected = 0;
		break;
								  ...
								  uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
	...
	if (att_handle == DEVICE_IP_ADDRESS_HANDLE)
	{
		printf("[ATT] Read request for device IP address: %s\n", wifi_setting.device_ip_address);
		return att_read_callback_handle_blob((const uint8_t *)&wifi_setting.device_ip_address,
											 strlen(wifi_setting.device_ip_address), offset, buffer,
											 buffer_size);
	}
	if (att_handle == SERVER_IP_ADDRESS_HANDLE)
	{
		printf("[ATT] Read request for server IP address: %s\n", wifi_setting.server_ip_address);
		return att_read_callback_handle_blob((const uint8_t *)&wifi_setting.server_ip_address,
											 strlen(wifi_setting.server_ip_address), offset, buffer,
											 buffer_size);
	}
	if (att_handle == SERVER_PORT_HANDLE)
	{
		printf("[ATT] Read request for server port: %s\n", wifi_setting.server_port);
		return att_read_callback_handle_blob((const uint8_t *)&wifi_setting.server_port,
											 strlen(wifi_setting.server_port), offset, buffer,
											 buffer_size);
	}
	if (att_handle == WIFI_SSID_USER_DESCRIPTION_HANDLE)
	{
		return att_read_callback_handle_blob((const uint8_t *)"SSID", 4, offset, buffer,
											 buffer_size);
	}
	if (att_handle == WIFI_PASSWORD_USER_DESCRIPTION_HANDLE)
	{
		return att_read_callback_handle_blob((const uint8_t *)"Password", 8, offset, buffer,
											 buffer_size);
	}
	if (att_handle == DEVICE_IP_ADDRESS_USER_DESCRIPTION_HANDLE)
	{
		return att_read_callback_handle_blob((const uint8_t *)"Device IP Address", 10, offset,
											 buffer, buffer_size);
	}
	if (att_handle == SERVER_IP_ADDRESS_USER_DESCRIPTION_HANDLE)
	{
		return att_read_callback_handle_blob((const uint8_t *)"Server IP Address", 10, offset,
											 buffer, buffer_size);
	}
	if (att_handle == SERVER_PORT_USER_DESCRIPTION_HANDLE)
	{
		return att_read_callback_handle_blob((const uint8_t *)"Server port", 10, offset, buffer,
											 buffer_size);
	}
							  ...
							  uint16_t buffer_size)
{
	...
	case WIFI_PASSWORD_HANDLE:
		...
		break;
	case SERVER_IP_ADDRESS_HANDLE:
		if (sizeof(wifi_setting.server_ip_address) < buffer_size)
		{
			printf("[ATT] WARN: Write message to server IP address is too long\n");
			return ATT_ERROR_INVALID_ATTRIBUTE_VALUE_LENGTH;
		}
		memcpy(wifi_setting.server_ip_address, buffer, buffer_size);
		wifi_setting.server_ip_address[buffer_size] = '\0';
		printf("[ATT] Server IP address: \"%s\"\n", wifi_setting.server_ip_address);
		break;
	case SERVER_PORT_HANDLE:
		if (sizeof(wifi_setting.server_port) < buffer_size)
		{
			printf("[ATT] WARN: Write message to server port is too long\n");
			return ATT_ERROR_INVALID_ATTRIBUTE_VALUE_LENGTH;
		}
		memcpy(wifi_setting.server_port, buffer, buffer_size);
		wifi_setting.server_port[buffer_size] = '\0';
		printf("[ATT] Server port: \"%s\"\n", wifi_setting.server_port);
		break;
	default:
		break;
	}
#ifdef DEBUG
	printf(CYAN "Using these values:\n");
	printf("SSID: %s\n", wifi_setting.ssid);
	printf("PWD: %s\n", wifi_setting.password);
	printf("Device IP: %s\n", wifi_setting.device_ip_address);
	printf("Server IP: %s\n", wifi_setting.server_ip_address);
	printf("Server port: %s\n", wifi_setting.server_port);
	printf("Check: %d\n", wifi_setting.rand_check_num);
	printf("sizeof(wifi_setting): %d Bytes\n" RESET, sizeof(wifi_setting));
#endif
	if (strlen(wifi_setting.ssid) > 0 && strlen(wifi_setting.password) > 0 &&
		strlen(wifi_setting.server_ip_address) > 0 && strlen(wifi_setting.server_port) > 0)
	{
		wifi_setting.rand_check_num = RAND_CHECK_NUM;
		flash_write(&wifi_setting, sizeof(wifi_setting_t));

#ifdef DEBUG
		wifi_setting_t wifi_setting2;
		flash_read(&wifi_setting2, FLASH_PAGE_SIZE);

		printf(GREEN "Reading values from flash:\n");
		printf("SSID: %s\n", wifi_setting2.ssid);
		printf("PWD: %s\n", wifi_setting2.password);
		printf("Device IP: %s\n", wifi_setting2.device_ip_address);
		printf("Server IP: %s\n", wifi_setting2.server_ip_address);
		printf("Server port: %s\n", wifi_setting2.server_port);
		printf("Check: %d\n", wifi_setting2.rand_check_num);
		printf("sizeof(wifi_setting): %d Bytes\n" RESET, sizeof(wifi_setting2));
#endif
	}
	else
	{
...
void wifi_task(void)
{
	...
	case DEVICE_WIFI_LINK_UP:
		...
		{
			char *device_ip_address = ip4addr_ntoa(&cyw43_state.netif[0].ip_addr);
			strcpy(wifi_setting.device_ip_address, device_ip_address);
			printf("[WIFI] Acquired IP address: %s\n", wifi_setting.device_ip_address);
			link_status = 1;
			LCD_print_next(" ", 0);
			LCD_print_next("Device address:", 0);
			strncpy(lcd_buff, wifi_setting.device_ip_address, strlen(wifi_setting.device_ip_address));
			lcd_buff[strlen(wifi_setting.device_ip_address)] = 0;
			LCD_print_next(lcd_buff, BLUE);
...
#define STR_HEADING "DEVICE CONFIG"

int start_ble(void)
{
	// stdio_init_all(); // Doing this in main()

	current_state = DEVICE_START_UP;

	printf("[MAIN] BLE Wi-Fi provisioning started\n");

	flash_read(&wifi_setting, sizeof(wifi_setting_t));

	Paint_DrawRectangle(2, 2, LCD_1IN3_WIDTH - 2, 28, BLACK, DOT_PIXEL_1X1, DRAW_FILL_FULL);
	Paint_DrawString_EN((LCD_1IN3_WIDTH - strlen(STR_HEADING) * Font20.Width) / 2, 7, STR_HEADING, &Font20, WHITE, BLACK);

	if (wifi_setting.rand_check_num != RAND_CHECK_NUM)
	{
		flash_erase();
		memset(&wifi_setting, 0, sizeof(wifi_setting_t));
		LCD_print_next("Config Wifi with BT", RED);
	}
	else
	{
		strncpy(lcd_buff, STR_SSID, strlen(STR_SSID));
		lcd_buff[strlen(STR_SSID)] = 0;
		strncat(lcd_buff, wifi_setting.ssid, sizeof(lcd_buff) - strlen(STR_SSID));

		LCD_print_next(lcd_buff, BLACK);
	}

#ifdef DEBUG
	sleep_us(3000); // so that this doesn't scroll past
	printf(PRINT_RED "Settings in flash:\n");
	printf("SSID: %s\n", wifi_setting.ssid);
	printf("PWD: %s\n", wifi_setting.password);
	printf("Device IP: %s\n", wifi_setting.device_ip_address);
	printf("Server IP: %s\n", wifi_setting.server_ip_address);
	printf("Server port: %s\n", wifi_setting.server_port);
	printf("Check: %d\n", wifi_setting.rand_check_num);
	printf("sizeof(wifi_setting): %d Bytes\n" PRINT_RESET, sizeof(wifi_setting));
#endif
	wifi_init();
	bluetooth_init();
